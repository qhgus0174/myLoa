...
const BackupModal = () => {
    ...
    const setBackupData = async () => {
        try {
            const {
                result: { todo, todoord, character, characterord, share, shareDay, ledger },
            } = (await (
            ...
            setStoredLedger(JSON.parse(ledger));
            setStoredShareContents(JSON.parse(share));
            setStoredDayContents(JSON.parse(shareDay));
        } catch (err: unknown) {
